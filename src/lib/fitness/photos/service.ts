...
export async function uploadPhoto(
  ...
  if (file.size > MAX_SIZE_BYTES) return { error: "Photo must be under 10MB" };
  if (angle !== "FRONT" && angle !== "SIDE" && angle !== "BACK") return { error: "Invalid angle" };
...
export async function deletePhoto(userId: string, photoId: string): Promise<SimpleResult> {
  ...
  if (!photo || photo.userId !== userId) return { error: "Not authorized" };
