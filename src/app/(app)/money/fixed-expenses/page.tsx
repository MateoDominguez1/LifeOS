  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <h1 className="font-display text-xl font-bold">{t.money.fixedExpenses.title}</h1>
        <Link
        ...
        >
          <Plus size={15} /> {t.money.fixedExpenses.newFixedExpense}
        </Link>
      </div>

      {expenses.length === 0 ? (
        <Card className="py-10 text-center text-sm text-ink-soft">{t.money.fixedExpenses.empty}</Card>
      ) : (
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-ink">
                    ...
                    {expense.name}
                    {pendingItem && (
                      <span className="rounded bg-warn-soft px-1.5 py-0.5 text-xs text-warn">{t.money.fixedExpenses.pending}</span>
                    )}
                  </div>
                  <CardLabel className="mt-1">
                    {t.money.common.dayLabel} {expense.dueDay} · {FREQUENCY_LABEL[expense.frequency]} · {expense.account.name}
                  </CardLabel>
                </div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    <form action={toggleFixedExpenseActiveAction.bind(null, expense.id)}>
                      <button
                        ...
                        className="rounded-lg px-2 py-1 text-xs font-medium text-ink-faint hover:bg-surface-raised hover:text-ink"
                      >
                        {expense.isActive ? t.money.common.deactivate : t.money.common.activate}
                      </button>
                    </form>
