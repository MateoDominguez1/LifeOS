  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <h1 className="font-display text-xl font-bold">Cuentas</h1>
      ...
      {accounts.length === 0 ? (
        <Card className="py-10 text-center text-sm text-ink-soft">Todavía no agregaste ninguna cuenta.</Card>
      ) : (
            ...
            <Card key={account.id} domain={account.isActive ? "money" : "neutral"} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {!account.isActive && (
                    <span className="rounded bg-border-soft px-1.5 py-0.5 text-xs text-ink-faint">Inactiva</span>
                  )}
                  {account.excludeFromTotal && (
                    <span className="rounded bg-accent-soft px-1.5 py-0.5 text-xs text-accent-ink">No cuenta</span>
                  )}
                </div>
