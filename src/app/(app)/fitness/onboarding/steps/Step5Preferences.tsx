...
export function Step5Preferences({
  ...
  return (
    <StepShell title="Preferencias" subtitle="Afinemos los detalles.">
      <Field label="Músculos prioritarios — opcional">
      ...
      <Field label="¿Te gusta el cardio?">
        <div className="grid grid-cols-2 gap-2">
          <Chip label="Sí" selected={data.cardioPreference} onClick={() => update({ cardioPreference: true })} />
          <Chip label="No" selected={!data.cardioPreference} onClick={() => update({ cardioPreference: false })} />
        </div>
      </Field>

      <Field label="¿Cómo preferís medir el esfuerzo?">
        <div className="grid grid-cols-3 gap-2">
          ...
          <Chip label="Ninguno" selected={data.rirRpeMode === "NONE"} onClick={() => update({ rirRpeMode: "NONE" })} />
        </div>
      </Field>

      <Field label="Ejercicios favoritos o a evitar — opcional">
        <TextInput type="text" placeholder="Buscar ejercicios..." value={query} onChange={(e) => setQuery(e.target.value)} />
