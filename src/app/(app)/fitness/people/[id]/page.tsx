      ...
      <div className="flex flex-col gap-4">
        <Card domain="fitness">
          <div className="flex items-center justify-between">
            ...
            <QuickAddManagedWeight managedProfileId={id} />
          </div>
          {latestWeight != null ? (
            <div className="mt-2 font-display text-2xl font-bold text-ink">{latestWeight} kg</div>
          ) : (
            <p className="mt-2 text-sm text-ink-soft">Todavía no hay peso registrado.</p>
          )}
          <div className="mt-3">
        ...
        <Card>
          <div className="flex items-center justify-between">
            ...
            <QuickAddManagedMeasurement managedProfileId={id} />
          </div>
          {measurements.length === 0 ? (
            <p className="mt-2 text-sm text-ink-soft">Todavía no hay medidas registradas.</p>
          ) : (
            <div className="mt-2 space-y-1">
              {measurements.map((m) => (
                <div key={m.id} className="flex justify-between text-sm">
                  <span className="text-ink">{m.type === "CUSTOM" ? m.customLabel ?? "Personalizado" : m.type}</span>
        ...
        <Card>
          <CardLabel>Récords personales</CardLabel>
          {records.length === 0 ? (
            <p className="mt-2 text-sm text-ink-soft">Todavía no hay récords registrados.</p>
          ) : (
            <div className="mt-2 space-y-1">
              {records.map((r) => (
