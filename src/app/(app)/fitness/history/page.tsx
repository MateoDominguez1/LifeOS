...
export default async function HistoryPage() {
  ...
  return (
    <div>
      ...
      {sessions.length === 0 ? (
        <Card className="text-center text-sm text-ink-soft">No hay entrenamientos completados todavía. Terminá tu primera sesión para verla acá.</Card>
      ) : (
            ...
            return (
              <Link key={s.id} href={`/fitness/history/${s.id}`}>
                <Card className="flex items-center justify-between transition-colors hover:bg-surface-raised">
                  <div>
                    <div className="text-sm font-medium text-ink">{s.workoutDay?.label ?? "Entrenamiento"}</div>
                    <div className="text-xs text-ink-faint">
                      {s.completedAt?.toISOString().slice(0, 10)} · {s.durationSec ? `${Math.round(s.durationSec / 60)} min` : "—"}
                    </div>
                  </div>
                  <div className="text-right text-xs text-ink-soft">
                    <div>{exerciseCount} ejercicios</div>
